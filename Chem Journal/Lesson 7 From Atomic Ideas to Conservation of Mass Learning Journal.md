---
layout: default
title: Lesson 7 From Atomic Ideas to Conservation of Mass Learning Journal
permalink: /chemistry/lesson-7-from-atomic-ideas-to-conservation-of-mass-journal/
section: Chemistry
summary: Independent learning record for Lesson 7 on early atomic ideas, quantitative evidence, conservation of mass, and composition laws.
...
我先问：为什么 atomos 只是“设想”，不能直接算作科学证据？因为它是关于物质由不可分粒子组成的解释，但当时没有定量测量来检验它。Boyle 的工作把研究推进到用实验和测量约束解释；Lavoisier 则用封闭系统记录反应前后的总质量。我的证据链是：古希腊 atomos 设想 → Boyle 的定量测量 → Lavoisier 的封闭系统质量账。
...
**Exercise 2.37**：肼、氨和叠氮化氢都只含氮和氢；每 $1.00\ \mathrm{g\ N}$ 分别结合 $1.44\times10^{-1}\ \mathrm{g\ H}$、$2.16\times10^{-1}\ \mathrm{g\ H}$ 和 $2.40\times10^{-2}\ \mathrm{g\ H}$。我固定氮为 $1.00\ \mathrm{g}$，所以直接比较氢的质量：
...
**Exercise 2.38**：两个各为 $100.0\ \mathrm{g}$ 的碳氧化合物，一个含 $27.2\ \mathrm{g\ C}$，另一个含 $42.9\ \mathrm{g\ C}$。不能直接用 $42.9/27.2$，因为两个样品中的氧质量不同。先求氧的质量：
...
我在这两题中容易卡在“到底固定谁”。只要题目给出相同的某元素质量，就固定它；如果没有相同质量，就先用总质量减去另一元素的质量，再把质量换算到相同基准。2.38 的 $2.01$ 不是直接把 $42.9$ 除以 $27.2$ 得到的。
...
**Exercise 2.40**：第一种化合物中 $14.0\ \mathrm{g\ R}$ 与 $3.00\ \mathrm{g\ Q}$ 化合；第二种中 $7.00\ \mathrm{g\ R}$ 与 $4.50\ \mathrm{g\ Q}$ 化合。固定 Q 的质量，先比较每 $1\ \mathrm{g\ Q}$ 所对应的 R：
...
**Exercise 2.41**：水分解的图示是 $2\mathrm{H_2O}\rightarrow2\mathrm{H_2}+\mathrm{O_2}$。反应前有 4 个氢原子和 2 个氧原子；反应后仍有 4 个氢原子和 2 个氧原子，只是原子的组合方式改变了。因此，在把所有物质都包括在系统中时，原子没有被创造或消灭，质量守恒。我的系统边界应包括反应容器及反应前后的全部物质；这题的理由不是“看起来一样”，而是逐个追踪原子数没有改变。
#### reflection
...
- How an open system can appear to lose mass when gas escapes.
- How fixing the mass of one element makes a mass ratio comparable.
- Why a small whole-number ratio is evidence for multiple proportions, not a claim that all substances have the same proportions.

## Mistakes and corrections

| Date | Topic | Mistake | Correction |
| --- | --- | --- | --- |
| 2026-09-03 | Multiple proportions | 只把不同化合物的原始质量直接相除，没有先固定一种元素的质量。 | 先固定相同质量的氧或氮，再比较另一元素的质量；如果基准不同，先换算到相同基准。 |
| 2026-09-03 | Conservation of mass | 看到敞口烧杯内质量变小，可能会把它理解成质量消失。 | 先画系统边界，并追踪逸出的气体和其他离开称量范围的物质。 |
| 2026-09-03 | Atomic ideas and evidence | 容易把 atomos 的解释直接当作已经证明的科学事实。 | 区分设想和定量证据：设想需要由可重复测量和系统边界明确的实验来检验。 |
