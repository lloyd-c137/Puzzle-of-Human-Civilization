---
layout: default
title: Biology Learning Journal
permalink: /biology/learning-journal/
section: Biology
summary: Overview of biology learning progress and Unit journals.
journal_type: overview
subject: Biology
---

# Biology Learning Journal

## Purpose

这是生物学科的总 Journal，用于查看整体进度、Unit 状态和当前重点。具体的理解、证据、疑问、错误与修正保存在对应的 Unit Journal 中。

这是学习记录，不代表整个生物学路径已经完成。课程界面的完成状态与真正能够解释、应用和回忆知识是分开记录的。

## Current overview

Unit 1 的自然系统、生命分布和生态位内容已经学习并整理；Unit 4 的 population growth、carrying capacity、limiting factors 和 r/K life-history strategies 已经开始记录。当前重点是继续 Unit 4 练习，并复习人口增长概念。

## Unit journals

| Unit | Main topics | Status | Last updated | Journal |
|---|---|---|---|---|
| Units 1 and 4 | Natural systems, ecology, niches, population growth, and carrying capacity | Exploring to Partly understood | 2026-08-24 | [Open Unit Journal]({{ '/biology/units-1-and-4-journal/' | relative_url }}) |

## Current learning focus

- 继续 Unit 4 exercises；
- 复习 carrying capacity 与 limiting factors 的关系；
- 区分 population 的当前变化和年龄结构对未来增长的影响；
- 用证据和自己的解释完成 retrieval check。

## Recurring mistakes

| Topic | Recurring mistake | Current correction |
|---|---|---|
| Community and ecosystem | 把 abiotic environment 也包含进 community。 | Community 是不同种群及其相互作用；ecosystem 还包括非生物环境。 |
| Population movement | 忽略 immigration 和 emigration 的方向。 | 移入增加数量，移出减少数量。 |
| Carrying capacity | 把 K 当成永远不变的精确上限。 | 环境条件变化时 K 会变化，population size 也可能在 K 上下波动。 |

## Recent progress

| Date | Unit | Topic | Progress | Next step |
|---|---|---|---|---|
| 2026-08-24 | Units 1 and 4 | Natural systems and population growth | Partly understood | Review the concepts and complete the next retrieval check. |
